...
module Util =
    ...
    let makeJsObject pairs =
        pairs |> Seq.map (fun (name, value) ->
            ...
            ObjectProperty(prop, value, computed_=computed) |> Choice1Of3)
        |> Seq.toArray
    ...
    let transformObjectExpr (com: IBabelCompiler) ctx (members: Fable.MemberDecl list) baseCall: Expression =
        let makeObjMethod kind prop computed hasSpread args body =
            ...
            ObjectMethod(kind, prop, args, body, computed_=computed,
                ?returnType=returnType, ?typeParameters=typeParamDecl) |> Choice2Of3
        let pojo =
            members |> List.collect (fun memb ->
                let info = memb.Info
                let prop, computed = memberFromName memb.Ident.Name
                if info.IsValue then
                    [ObjectProperty(prop, com.TransformAsExpr(ctx, memb.Body), computed_=computed) |> Choice1Of3]
                    ...
                    let iterator =
                        ...
                        ObjectMethod(ObjectMeth, prop, [||], body, computed_=computed) |> Choice2Of3
                    [method; iterator]
                else
    ...
    let declareType (com: IBabelCompiler) ctx (ent: Fable.Entity) ident (consArgs: Pattern[]) (consBody: BlockStatement) baseExpr classMembers: Choice<Statement, ModuleDeclaration> list =
